...
describe('SduiParser Engine Validation', () => {
  ...
  describe('Root Manifest Integrity', () => {
    ...
    it('Throws if root blade is not fundamentally a Blade type', () => {
      expect(() => SduiParser.parseManifest({ version: "1.0", blade: { type: "Sdui.Display.Text" } })).toThrowError(/Root node must be of type/);
    });
  });
  ...
  describe('Strict Component Taxonomy Edge-Cases', () => {
    ...
    it('Rejects a DataGrid missing the "columns" array', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.DataGrid, properties: { rows: [] } }))
        .toThrowError(/DataGrid strictly requires a "columns" array/);
    });
    it('Rejects a DataGrid missing the "rows" array', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.DataGrid, properties: { columns: [] } }))
        .toThrowError(/DataGrid strictly requires a "rows" array/);
    });
    it('Approves a flawless DataGrid', () => {
      expect(SduiParser.parseNode({ type: SduiElementType.DataGrid, properties: { columns: [], rows: [] } })).toBeDefined();
    });

    // Pivot
    it('Rejects a Pivot tab missing the "items" definition', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.Pivot, properties: {} }))
        .toThrowError(/Pivot strictly requires an "items" array/);
    });
    it('Approves a flawless Pivot', () => {
      expect(SduiParser.parseNode({ type: SduiElementType.Pivot, properties: { items: [{title: 'A', targetId: '1'}] } })).toBeDefined();
    });

    // CommandBar
    it('Rejects a CommandBar missing items', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.CommandBar, properties: {} }))
        .toThrowError(/CommandBar strictly requires an "items" array/);
    });

    // Grid Layout
    it('Rejects a Grid missing numeric column limits', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.Grid, properties: { gap: '10px' } }))
        .toThrowError(/Grid strictly requires a numeric "columns"/);
    });

    // Dropdown
    it('Rejects a Dropdown missing internal options', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.Dropdown, properties: { label: 'Sort' } }))
        .toThrowError(/Dropdown strictly requires an "options" array/);
    });

    // StatusBadge
    it('Rejects a StatusBadge missing status constants', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.StatusBadge, properties: { text: "Online" } }))
        .toThrowError(/StatusBadge strictly requires "status" and "text"/);
    });

    // Alert
    it('Rejects an Alert lacking severity typing', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.Alert, properties: { message: "Failed" } }))
        .toThrowError(/Alert strictly requires "type" and "message"/);
    });

    // ExtensionMount
    it('Rejects an ExtensionMount without an expected routing point', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.ExtensionMount, properties: {} }))
        .toThrowError(/ExtensionMount strictly requires a "mountPoint"/);
    });

    // InvokeControl
    it('Rejects an InvokeControl lacking action payloads', () => {
      expect(() => SduiParser.parseNode({ type: SduiElementType.InvokeControl, properties: { label: "Save" } }))
