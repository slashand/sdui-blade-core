    ...
    return createStore<BladeState>()((set) => ({
        ...
        openBlade: (blade) => {
            ...
            set((state) => {
                ...
                return { 
                    activeBlades: [...state.activeBlades, { ...blade, id, properties: blade.properties || {} } as Required<SduiBlade>] 
                };
            });
        },
        ...
        closeBlade: (id, force = false) => {
            set((state) => {
                ...
                if (!force && blade && blade.properties?.isDirty) {
                    console.warn(`[SDUI Guard]: Attempted to close dirty blade [${id}]. Close rejected. Use force=true to override.`);
                    return state;
                }
        ...
        closeTopBlade: (force = false) => {
            set((state) => {
                ...
                if (!force && top.properties?.isDirty) {
                    console.warn(`[SDUI Guard]: Attempted to close dirty top blade [${top.id}]. Close rejected. Use force=true to override.`);
                    return state;
                }
        ...
        closeAllBlades: (force = false) => {
            set((state) => {
                if (!force && state.activeBlades.some(b => b.properties?.isDirty)) {
                    console.warn(`[SDUI Guard]: Attempted to close all blades, but one or more are dirty. Close rejected. Use force=true to override.`);
                    return state;
                }
                return { activeBlades: [] };
            });
